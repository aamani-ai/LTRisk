...
| [07](C_financial_translation/07_hcr_hazard_change.md) | **HCR: Hazard Change Ratio** | Change in expected annual damage per canonical hazard (frequency + severity). 10 hazards by tier (computable/published/blocked), published scaling vs direct computation, non-linear amplification (2.5× vs 26×), BI connection (Additional_BI = baseline_BI × HCR), linearity assumption, sensitivity analysis, negative HCR (warming benefits) |
